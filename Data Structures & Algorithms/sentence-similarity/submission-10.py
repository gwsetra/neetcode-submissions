    ...
    def areSentencesSimilar(self, sentence1: List[str], sentence2: List[str], similarPairs: List[List[str]]) -> bool:
        ...
        if len(sentence1) != len(sentence2):
            ...
            for iters in range(len(sentence1)):
                for i in range(len(shortpairs)):
                    # print(similarPairs[i])
                    if sentence1[iters] == sentence2[iters]:
                        cnt+=1
                        # print('found')
                        break
                    if (shortpairs[i][0] == sentence1[iters] or shortpairs[i][0] == sentence2[iters]) and (shortpairs[i][1] == sentence1[iters] or shortpairs[i][1] == sentence2[iters]):
                        cnt+=1
                        break
