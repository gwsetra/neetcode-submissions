class Solution:
    def orangesRotting(self, grid: List[List[int]]) -> int:
        ...
        def bfs(row, count):
            ...
            print('start bfs', row, count)
            queue.append((row, count))
            visited.add((row, count))
        ...
        if not fresh_checker: # if there's no 1 or fresh fruite
            return 0
        else:
            return num_of_minutes if fresh_checker.issubset(visited) or fresh_checker else -1
